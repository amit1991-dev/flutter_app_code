...
class QuestionStatus {
  ...
  QuestionStatus({required this.index,required this.question,this.answer="",this.tempAnswer="",this.optional=false,this.timeSpent=0});

  QuestionStatus.fromJson(Map<String,dynamic> json,this.question,{this.index=0,TestSection? ts,}){
    ...
    // The bug was present in the following line
    // tempAnswer = answer;
    // To fix this, assign tempAnswer only if astate is ANSWERED_SAVED
    // if (json["attempted"] != null && json['attempted']) {
    //   tempAnswer = answer;
    // }

    // // Assign tempAnswer based on astate
    // if (json["attempted"] != null) {
    //   if (json['attempted']) { 
    //     tempAnswer = answer; // User saved the answer
    //   } else {
    //     tempAnswer = ""; // User interacted but didn't save
    //   }
    // } else {
    //   tempAnswer = ""; // Question not attempted yet
    // }
  ...
  @override
  bool operator ==(other) => other is QuestionStatus && (other.state == state);
