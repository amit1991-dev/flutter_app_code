...
class TestStateController extends GetxController {
  late TestContext testContext;
  late final StudentsAPIProvider studentsAPIProvider;
  late TestWithSections testWithSections;
  late Timer testTimer;
  late List<QuestionStatus> selected;//initialize this one
  ...
  TestStateController({required this.testContext}){
  ...
  void timerFunction() async{
    ...
    if(testState == TestState.STARTED)
    {
      ...
      //   syncServer();
      // }
       // this line control the time of syncserver 
// If you change it to time.value % 20 == 0, then syncServer() will be called every 20 seconds.
       if(time.value%20==0){ 
        syncServer();
      }
  ...
  Future<bool> submitTest() async {
    if(testState != TestState.FINISHED){
      ...
      if(await syncServer()){
        ...
        APIResponse res=await studentsAPIProvider.submitTestState(testContext);
        ...
        if(res.status == TextMessages.SUCCESS){
          ...
          // Get.back();
          Get.toNamed(Routes.POST_TEST_PAGE,arguments:{"test_state_id":testContext.id,"result_id":res.data});
  ...
  Future<bool> syncServer() async {
    ...
    APIResponse res=await studentsAPIProvider.updateTestState(testContext);
    if(res.status == TextMessages.SUCCESS){
