...
import '../../../../../data/model/gravity/subject.dart';
...
class FileSubjectsPage extends StatefulWidget {
  FileSubjectsPage({Key? key}):super(key: key){
    // Get.put<HostController>(HostController());
  }
  @override
  State<FileSubjectsPage> createState() => _State();
}

class _State extends State<FileSubjectsPage> {
  late String batchId;
  bool agreed = false;
  _State(){
    ...
    return SafeArea(
      child: Material(
        ...
        child: AsyncBuilder<APIResponse>(
          ...
          future: Get.find<StudentsController>().getSubjectsForFiles(batchId),
          waiting: (context) {
            return  Center(
                          ...
                          child: SingleChildScrollView(
                            child: Column(
                              children: [
                                ...
                                if(subjects.length==0) clumsyTextLabel("Sorry, there are currently no files available in this Batch!"),
                                ...List.generate(subjects.length, (index) {
                                  // return gravityTestsListTile(subjects[index]);
                                  return ListTile(
                                    onTap: (){
                                      Get.toNamed(Routes.BATCH_FILE_SUBJECT_CHAPTERS_PAGE,arguments: {"subject":subjects[index].id,"batch":batchId});
                                    },
